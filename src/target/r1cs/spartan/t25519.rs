...
use libdoriant25519::{
    scalar::Scalar, Assignment, InputsAssignment, Instance, NIZKGens, VarsAssignment, NIZK,
};
...
pub fn precompute_inner(
    ...
    for var in prover_data.r1cs.vars.iter() {
        assert!(matches!(
            var.ty(),
            VarType::Inst | VarType::Chall | VarType::FinalWit | VarType::RoundWit
        ));
        match var.ty() {
...
pub fn precompute(prover_data: &ProverData) -> io::Result<(NIZKGens, Instance)> {
